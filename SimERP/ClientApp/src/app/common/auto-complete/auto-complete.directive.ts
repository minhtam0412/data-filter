...
export class NguiAutoCompleteDirective implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  @Input('max-Height') public maxHeight = 250;
  @Input('is-rtl') public isRtl = false;
  @Input('dropdown-custom-class') public dropdowncustomclass = '';
  @Input('dropdown-custom-width') public dropdowncustomwidth;
  ...
  public showAutoCompleteDropdown = (event?: any): void => {
    ...
    component.dropdowncustomclass = this.dropdowncustomclass;
  ...
  public styleAutoCompleteDropdown = () => {
    if (this.componentRef) {
      ...
      // nếu không chỉ định width, dropdown sẽ lấy width theo component cha
      if (!this.dropdowncustomwidth) {
        this.acDropdownEl.style.width = thisInputElBCR.width + 'px';
      } else {
        this.acDropdownEl.style.width = this.dropdowncustomwidth + 'px';
      }
      ...
      this.acDropdownEl.style.maxHeight = this.maxHeight + 'px';;
      this.acDropdownEl.style.overflowY = "scroll";
